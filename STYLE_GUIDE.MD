...
The goal of this document is to help keep our codebases consistent and maintainable by outlining best-practices we've
learned through experience. It is currently a mix of best practices for _this codebase_ (ie. how we expect code to
be organized), and best practices for _Rust in general_. The latter is mostly motivated by issues we seen enough to
...
CI runs clippy's default lint set across the whole workspace and treats all warnings as errors (`just check` runs
the same checks locally). If a warning or clippy lint is firing for your code, strongly consider fixing it. Avoid
using `#[allow(...)]` unless you have a strong reason to do so. New code should generally not have to `#[allow]`
any lints or warnings.
...
- When defining a new RPC whose main purpose is to perform the same operation for more than one node, name it with the
  `Batch` prefix. For example, prefer names shaped like `BatchSetPowerState`, `BatchGetPowerState`, and
  `BatchUpdateFirmware` over pluralized one-off names.
- Multi-node RPCs should return a response message that includes `NodeBatchResponse response = 1;`.
  Put operation-specific success data in additional fields, such as per-node state payloads or per-node job
  IDs.
- Populate `NodeBatchResponse` consistently:
    - `status` is `RETURN_CODE_SUCCESS` only when every targeted node completed or was accepted successfully.
    - `message` is a human-readable summary of the batch operation.
    - `node_results` contains per-node success and failure details.
    - `job_id` contains the parent job ID for asynchronous batch work. For synchronous batch RPCs, leave it empty.
    - `stats` contains aggregate node counts.
- For asynchronous multi-node APIs, return an immediate failure only when the request fails initial validation, such as
  malformed input, missing required fields, invalid target selection, or no jobs being created.
- After initial validation succeeds, create and return a parent `job_id`. If individual nodes fail or are rejected after
  that point, report those failures through `NodeBatchResponse.node_results`, the operation-specific per-node job list,
  and the matching job-status RPC instead of failing the whole request synchronously.

### Declarative style APIs

RMS should prefer declarative style APIs over imperative style APIs.
This means instead of saying "perform exactly a certain change", RMS should allow its users to express desired state (e.g. firmware version or power state).
After accepting a declarative API call, RMS should determine whether the target state is already met:

- If it is, skip any actions and report success
- If not, perform the minimum changes required to meet the target state.

Whether an API successfully meets the declarative nature criteria can be measured as follows:

- An RMS client should be able to call the respective API in a loop. E.g.
  
    ```rust
    // Keep the fabric in the desired state - even across switch restarts or reinstalls
    loop {
        job_id = rms_client.set_scaleup_fabric_state(desired_state);
        wait_for_job_success(rms_client, job_id);
    }
    ```

- Calling the API in a loop should not have any negative side effects like an intermittent loss of connectivity on networking
  (that would occur if RMS would perform actions without checking first if they are necessary)

**==> This pattern allows RMS clients to implement reconciliation loops and "self healing" while the complexity of the implementation lives within RMS.**
...
Use `///` to begin document code (doc-comments above functions, structs, etc), and `//` to comment inline.

When writing doc-comments, use markdown syntax.
...
Example doc-comment above function.
...
See [Making Useful Documentation Comments](https://doc.rust-lang.org/book/ch14-02-publishing-to-crates-io.html#making-useful-documentation-comments) some good examples.
...
you're awaiting is blocked or slow.

## Database wrappers

- Type definitions: Separate models from database calls.
...
Prefer a single `JoinSet` that spawns all background tasks, with `join_all()` blocking "forever" until
...
cancellation token from the top-level startup path, and stop your work when that token is
cancelled.
...
the task when dropped. With many such client-less background jobs, storing each of their
