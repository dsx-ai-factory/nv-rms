    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    /// Add span attribute keys to emit in event logs.
    ///
    /// Only keys passed here are promoted from spans onto event lines. Lookup
    /// walks from the current span through parent spans so nested operation logs
    /// can inherit job context without copying those fields onto every child
    /// span. The current span takes precedence over parent spans when both
    /// define the same key.
    ...
    /// Whether event log lines include a `time=<rfc3339>` field (see
    /// `write_event_data`). Defaults to `false` so ad hoc/test layers don't
    /// have to account for a nondeterministic field on every line; the
    /// production stdout layer built in `setup_logging` passes through the
    /// `enable_timestamps` config value (also defaulting to `false`).
    pub fn with_event_timestamps(self, emit_timestamps: bool) -> Self {
        Self {
            emit_timestamps,
            ..self
        }
    ...
    S: Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    fn on_event(&self, event: &Event<'_>, ctx: Context<'_, S>) {
        ...
        fn write_event_data<'a, R: LookupSpan<'a>>(
            ...
            fn span_attribute<'a, R: LookupSpan<'a>>(
                span: &SpanRef<'a, R>,
                key: &str,
            ) -> Option<String> {
                let ext = span.extensions();
                let data = ext.get::<LogFmtData>()?;

                data.attributes.get(key).cloned()
            }

            write!(out, "level={} ", event.metadata().level())?;
            if emit_timestamps {
                write!(out, "{} ", kvp("time", format_event_timestamp()))?;
            }
            ...
            if let Some(current_span) = current_span {
                ...
                if let Some(span_id) = span_attribute(current_span, "span_id") {
                    write!(out, "{} ", kvp("span_id", span_id))?;
                }

                // Extra event fields can be owned by parent job spans while the
                // event is emitted from a nested operation span.
                for key in extra_event_fields {
                    if event.metadata().fields().field(key).is_some() {
                        continue;
                    }

                    if let Some(value) = current_span
                        .scope()
                        .find_map(|span| span_attribute(&span, key))
                    {
...
/// Formats the current wall-clock time for the `time=` event field.
///
/// Extracted from `write_event_data` so its RFC3339 formatting can be unit
/// tested directly, independent of whether `LogFmtLayer::emit_timestamps`
/// causes it to actually be written.
fn format_event_timestamp() -> String {
...
impl<K: AsRef<str>, V: AsRef<str>> std::fmt::Display for Kvp<K, V> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
            ...
            .any(|c| *c <= b' ' || matches!(*c, b'=' | b'"'))
        {
            ...
            write!(f, "{}={}", escaped_key, value.escape_debug())?;
        }
...
mod tests {
    ...
    fn capture_event_line_with_fields(fields: &[&str], emit: impl FnOnce()) -> String {
        let writer = TestWriter::new();
        let cloned_writer = writer.clone();
        let layer = layer()
            .with_writer(Arc::new(move || Box::new(cloned_writer.clone())))
            .with_span_logs(false)
            .with_event_fields(fields.iter().map(|field| field.to_string()).collect());
        ...
        let line = lines
            ...
            .unwrap_or_else(|| panic!("logfmt output was empty"));

        assert!(lines.next().is_none(), "expected one logfmt line: {output}");

        line
    }

    /// Removes the nondeterministic `time=<rfc3339>` field from an event line so
    /// the remainder can be compared against a fixed expected string.
    ///
    /// The `time` field is always written immediately after `level=<LEVEL> `
    /// (see `write_event_data`), so this looks for it there rather than
    /// searching the whole line for the substring `time=`, which could
    /// spuriously match inside a quoted field value (e.g. `msg="elapsed
    /// time=5s"`).
    fn strip_time_field(line: &str) -> String {
        let Some(after_level) = line.find(' ').map(|idx| idx + 1) else {
            return line.to_string();
        };
        let rest = &line[after_level..];
        if !rest.starts_with("time=") {
            return line.to_string();
        }
        let field_len = rest.find(' ').unwrap_or(rest.len());
        let mut stripped = line[..after_level].to_string();
        stripped.push_str(rest[field_len..].trim_start());
        stripped
    }

    #[test]
    fn format_event_timestamp_produces_rfc3339_micros() {
        use chrono::SubsecRound;

        // Tested directly (independent of `LogFmtLayer::emit_timestamps`,
        // which defaults to `false`) so the formatting itself stays covered
        // even though it's not exercised end-to-end via `on_event` unless a
        // layer opts in with `with_event_timestamps(true)`.
        //
        // `before` is truncated to microsecond precision (matching
        // `to_rfc3339_opts(SecondsFormat::Micros, ...)`, which truncates
        // rather than rounds) so the comparison below can't spuriously fail
        // when `before` and the formatted timestamp land in the same
        // microsecond but `before` has later sub-microsecond nanos.
        let before = chrono::Utc::now().trunc_subsecs(6);
        let time_value = format_event_timestamp();
        let after = chrono::Utc::now();

        let parsed = chrono::DateTime::parse_from_rfc3339(&time_value)
            .unwrap_or_else(|e| panic!("time value '{time_value}' is not RFC3339: {e}"))
            .with_timezone(&chrono::Utc);

        assert!(
            parsed >= before && parsed <= after,
            "timestamp {parsed:?} not within [{before:?}, {after:?}]"
        );
    }

    #[test]
    fn test_event_time_field_is_omitted_unless_enabled() {
        let writer = TestWriter::new();
        let cloned_writer = writer.clone();
        let layer = layer()
            .with_writer(Arc::new(move || Box::new(cloned_writer.clone())))
        ...
        tracing::info!("untimestamped_event");
        ...
        assert!(
            ...
            "expected no time= field, got: {}",
            lines[0]
        );
    }

    #[test]
    fn test_event_time_field_is_present_when_enabled() {
        let writer = TestWriter::new();
        let cloned_writer = writer.clone();
        let layer = layer()
            .with_writer(Arc::new(move || Box::new(cloned_writer.clone())))
            .with_span_logs(false)
            .with_event_timestamps(true);

        let _subscriber = tracing_subscriber::registry().with(layer).set_default();

        tracing::info!("timestamped_event");

        let lines: Vec<String> = writer.text().lines().map(ToString::to_string).collect();
        assert_eq!(lines.len(), 1, "Expected 1 line, got {:?}", lines);
        assert!(
            strip_time_field(&lines[0]).starts_with(r#"level=INFO msg=timestamped_event"#),
            "expected a time= field immediately after level=, got: {}",
            lines[0]
        );
    }
    ...
    fn test_logfmt_layer() {
        ...
        assert!(strip_time_field(&lines[0]).starts_with(r#"level=INFO span_id=s1234 bool_value=false debug_value=111s error_value=abc f64_value=-2.0 i64_value=-3 str_escaped_value="Hello World" str_value=Hello summary=Summary1 location=""#), "Line is: {}", lines[0]);
        assert!(
            strip_time_field(&lines[1])
                .starts_with(r#"level=ERROR span_id=s1234 msg="as expected" location=""#),
            "Line is: {}",
            lines[1]
        );
    ...
    fn test_span_logs_are_emitted_according_to_log_level() {
        ...
        assert!(
            strip_time_field(&lines[0])
                .starts_with(r#"level=WARN span_id=s1234 msg=warn_event location="#),
            "Line is: {}",
            lines[0]
        );
        assert!(
    ...
    fn test_disable_span_logs() {
        ...
        assert!(
            strip_time_field(&lines[0])
                .starts_with(r#"level=INFO span_id=s1234 msg=abc location=""#),
            "Line is: {}",
            lines[0]
        );
    }
    ...
    fn test_event_outside_of_span() {
        ...
        assert!(
            strip_time_field(&lines[0])
                .starts_with(r#"level=WARN msg=outside_event! a=100 location=""#),
            "Line is: {}",
            lines[0]
        );
    }
    ...
    fn test_object_id_in_event_logs() {
        ...
        assert!(
            strip_time_field(&lines[0]).starts_with(r#"level=INFO span_id=s1234 object_id=machine-abc-123 msg="event inside span with object_id" location=""#),
            "Line is: {}",
            lines[0]
        );
    }
    ...
    #[test]
    fn event_fields_include_configured_parent_span_attributes() {
        let line = capture_event_line_with_fields(&["job_id"], || {
            ...
            let upload_span = tracing::info_span!("switch_system_image_sftp_upload");
            let _upload_entered = upload_span.enter();

            tracing::info!("SFTP upload progress");
        });

        assert!(
            line.contains("job_id=job-123"),
            "line did not include parent job_id: {line}"
        );

        assert!(
            !line.contains("node_id="),
            "line should not include unconfigured node_id: {line}"
        );
    }

    #[test]
    fn event_fields_prefer_current_span_attributes_over_parent_span_attributes() {
        let line = capture_event_line_with_fields(&["job_id"], || {
            let job_span = tracing::info_span!("parent_job", job_id = "parent-job");
            let _job_entered = job_span.enter();
            let upload_span = tracing::info_span!("child_upload", job_id = "child-job");
            let _upload_entered = upload_span.enter();
        ...
        assert!(
            line.contains("job_id=child-job"),
            "line did not prefer current span job_id: {line}"
        );
    }

    #[test]
    fn event_fields_do_not_duplicate_event_attributes() {
        let line = capture_event_line_with_fields(&["job_id"], || {
            let job_span = tracing::info_span!("switch_system_image_update", job_id = "span-job");
            let _job_entered = job_span.enter();

            tracing::info!(job_id = "event-job", "stage starting");
        });

        assert!(
            line.matches("job_id=").count() == 1 && line.contains("job_id=event-job"),
            "line should retain event job_id exactly once: {line}"
        );
    }

    #[test]
    fn kvp_redacts_secret_like_content_in_span_attributes() {
        let writer = TestWriter::new();
        let cloned_writer = writer.clone();
        let layer = layer().with_writer(Arc::new(move || Box::new(cloned_writer.clone())));
        ...
        let span = tracing::span!(
            ...
            "request",
            span_id = "req-1",
            password = tracing::field::Empty,
            otel.status_message = tracing::field::Empty,
            rpc.grpc.status_description = tracing::field::Empty,
        );
        let _entered = span.enter();
        span.record("password", "s3cr3t");
        span.record(
            "otel.status_message",
            "southbound failed: password=plain_secret",
        );
        span.record(
            "rpc.grpc.status_description",
            r#"Code: Internal, Message: BMC echoed password=plain_secret"#,
        );
        drop(_entered);
        drop(span);
        ...
        let output = writer.text();
        assert!(
            !output.contains("plain_secret"),
            "secret must be redacted in log output: {output}"
        );
        assert!(
            !output.contains("s3cr3t"),
            "plain secret value must be redacted when key is password: {output}"
        );
        assert!(
            output.contains("XXXX"),
            "redacted placeholder expected in log output: {output}"
        );
    }
}
