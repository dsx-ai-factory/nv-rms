...
/// Default `gateway_id` sent on NMX-C gRPC requests from RMS. Used as the serde
/// default for the `nmx_gateway_id` configuration field.
pub const DEFAULT_NMX_C_GATEWAY_ID: &str = "rack-manager-grpc-client";
pub const NMX_C_FM_CONFIG_FILE: &str = "fm_config";
pub const NMX_C_SM_CONFIG_FILE: &str = "sm_config";
...
pub(crate) use response::static_config_file_values;
...
impl NmxcClientPool {
    ...
    pub async fn create_client(
        ...
        Ok(Box::new(NmxcApi::new(channel, self.timeout)))
    }
    ...
    async fn connect(
        ...
        // Bound the whole connection attempt with a single deadline. Unlike
        // tonic's `connect_timeout` (which only covers the TCP connect), this
        // caps TCP + TLS + HTTP/2 handshake together, so a stall anywhere in
        // that sequence can't hang here indefinitely.
        let connect = async {
            if use_tls {
        ...
        let channel = tokio::time::timeout(self.timeout, connect)
            .await
            .map_err(|_| {
                tracing::warn!(uri = %uri, timeout_secs = self.timeout.as_secs(), "gRPC transport connect timed out");
                NmxcError::Status(tonic::Status::deadline_exceeded(format!(
                    "connect to NMX-C at {uri} timed out after {} seconds",
...
pub trait Nmxc: Send + Sync + 'static {
    ...
    /// Read one NMX-C static configuration value.
    ...
    /// Read complete NMX-C static configuration files.
