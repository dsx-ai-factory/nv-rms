...
impl SshClient {
    ...
    pub async fn sftp_upload_with_options(
        ...
        with_sftp_timeout(host, remote_path, "SFTP upload", upload_timeout, async {
            let step_timeout = options.step_timeout;

            let sftp = with_sftp_timeout(
                host,
                remote_path,
                "SFTP session setup",
                step_timeout,
            ...
            let mut remote_file = with_sftp_timeout(
                host,
                remote_path,
            ...
            log_sftp_upload_completed(host, local_path, remote_path, &snapshot, buffer_size_bytes);
...
/// Apply a timeout to an SFTP operation.
///
/// The upload path uses this twice: the caller-supplied upload timeout caps the
/// whole transfer, while [`SftpUploadOptions::step_timeout`] caps each setup,
/// open, read, write, and flush operation so stalls are detected promptly.
...
mod tests {
    ...
    fn validate_no_sftp_deferred_payload_rejects_data_messages() {
        let err = validate_no_sftp_deferred_payload(
            "/remote/fw.bin",
            vec![ChannelMsg::Data {
                data: CryptoVec::from_slice(b"payload"),
            }],
        )
        .unwrap_err();
    ...
    #[tokio::test]
    async fn with_sftp_timeout_returns_completed_value() {
        let value = with_sftp_timeout(
            "10.0.0.1",
            "/remote/fw.bin",
            "SFTP remote file write",
    ...
    #[tokio::test]
    async fn with_sftp_timeout_rejects_stalled_future() {
        let err = with_sftp_timeout(
            "10.0.0.1",
            "/remote/fw.bin",
            "SFTP remote file write",
