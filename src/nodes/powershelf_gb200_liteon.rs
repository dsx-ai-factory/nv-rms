...
impl Node for PowershelfGb200Liteon {
    ...
    async fn set_power_state(&self, op: PowerOp, _target: PowerTargetType) -> Result<()> {
        ...
        let intent = match op {
            ...
            PowerOp::Off | PowerOp::ForceOff => ChassisIntent::ForceOff,
            PowerOp::PowerCycle
...
async fn invoke_chassis_action(
    ...
    // ActionInfo's AllowableValues may not list every value a BMC actually
    // accepts, so warn rather than reject when our intent isn't advertised
    // and let the BMC make the final call.
    if !reset.action_info.is_empty() {
        ...
        if let Some(allowed) = reset_allowable_values(&info) {
            if !allowed.iter().any(|v| v == intent.redfish_reset_type()) {
                tracing::warn!(
                    intent = intent.redfish_reset_type(),
                    allowed = ?allowed,
                    "#Chassis.Reset ResetType not in AllowableValues; attempting anyway"
                );
            }
        }
    }
