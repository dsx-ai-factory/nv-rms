...
use crate::orchestrator::job_lifecycle::JobFailure;
use crate::orchestrator::job_tracker::{JobError, JobType, TrackedRmsJob};
...
pub(crate) fn create_switch_system_image_node(
    ...
    match NodeInstance::from_config(config, rack_id)? {
        NodeInstance::SwitchGb200Nvidia(node) => {
...
pub(crate) async fn run_switch_system_image_job(
    ...
    let stage_image_started = Instant::now();
    {
        ...
        if let Err(e) = sw
            .as_ref()
            .push_system_image_file(local_file_path, image_filename, sftp_upload_options)
            .await
        {
            tracing::error!(
                job_id,
                ...
                local_file_path,
                image_filename,
                "stage failed: SFTP upload / verification failed"
            );
            timeline.fail(
                "stage_image",
                &e.message,
    ...
    if !image_visible_before_stage && stage_image_duration_ms > 0 {
        if let Some(size) = local_image_size_bytes {
