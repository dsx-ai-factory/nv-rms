...
use crate::domain::node::{
    FirmwareActivationMode, FirmwareActivationRequest, FirmwareTarget, FirmwareTaskStatus,
    FirmwareType, FirmwareUpdateOptions, FirmwareUpdateOutcome, FirmwareUpdateSummary,
    FirmwareVersionCheckSummary, Node, NodeKind, NodeType,
};
use crate::domain::rack::NodeConfig;
use crate::nodes::NodeInstance;
use crate::orchestrator::job_tracker::{
    JobError, JobTracker, JobType, PendingRmsJob, TrackedRmsJob, job_error_of,
};
    ...
    loop {
        ...
        match node.poll_firmware_task(task_id).await {
            ...
            Err(e) if is_retryable_firmware_poll_error(&e) => {
                ...
                let retry_deadline = retry_started_at + FIRMWARE_TRANSIENT_POLL_ERROR_RETRY_TIMEOUT;
...
fn activation_mode_for_node<N: Node>(node: &N) -> Result<FirmwareActivationMode> {
    match node.node_type() {
        ...
        | NodeType::ComputeGb300Lenovo => Ok(FirmwareActivationMode::FullGb200Compute),
        NodeType::SwitchGb200Nvidia | NodeType::SwitchGb300Nvidia => {
            Ok(FirmwareActivationMode::SwitchPowerCycle)
        }
...
pub(crate) fn spawn_firmware_update_job<N: Node + Send + Sync + 'static>(
    ...
    tracker
        .spawn_tracked(pending, move |job| async move {
            ...
            let mut post_activation_check = None;
            if activate && !all_skipped {
                job.progress("Activating firmware");
                if let Err(e) = activate_firmware_for_node(node.as_ref()).await {
                    let message = format!("Firmware flashed but activation failed: {}", e.message);
                    fail_tracked_firmware_job(job, "activation", None, JobError::Other, &message);
                    return;
                }

                if node_supports_post_activation_version_check(node.as_ref()) {
                    job.progress("Verifying activated firmware versions");
                    match post_activation_version_check(node.as_ref(), &applied_targets).await {
                        Ok(summary) => {
                            post_activation_check = summary.map(|summary| summary.details);
                        }
                        Err(e) => {
                            fail_tracked_firmware_job(
                                job,
            ...
            let result = serde_json::json!({
                ...
                "activated": activate && !all_skipped,
...
impl RackManagerServiceImpl {
    ...
    > {
        ...
        let target_type = match proto_node_type_to_domain(r.node_type) {
            ...
            None => {
                ...
                batch.message = "unknown node type".into();
        ...
        // Spawn one background job per matching node, collect child IDs for parent
        ...
        for node in rack.list_nodes() {
            ...
            let tracker = self.job_tracker.clone();
            spawn_firmware_update_job(
                tracker,
                pending,
                node,
        ...
        let parent_id = self
            .job_tracker
            .create_parent_job(&r.rack_id, &child_ids, JobType::FirmwareUpdate)
            .unwrap_or_default();
    ...
    pub(crate) async fn handle_batch_update_firmware(
        ...
        for (nt_key, target_list) in &r.firmware_targets {
            let Some(node_type) = proto_node_type_to_domain(*nt_key) else {
                batch.message =
                    format!("Failed to resolve firmware targets for unknown node type {nt_key}");
                return Ok(tonic::Response::new(batch_update_firmware_response(
                    batch,
                    jobs,
                    total_nodes,
            ...
            match build_firmware_targets_from_list(
                ...
                Ok(targets) if targets.is_empty() => {
                    batch.message = format!(
                        "Failed to resolve firmware targets for node type {nt_key}: empty list"
                    );
                    return Ok(tonic::Response::new(batch_update_firmware_response(
                        batch,
                        jobs,
                ...
                Err(e) => {
                    batch.message =
                        format!("Failed to resolve firmware targets for node type {nt_key}: {e}");
                    return Ok(tonic::Response::new(batch_update_firmware_response(
                        batch,
                        jobs,
                        total_nodes,
        ...
        let mut parent_rack_id: Option<String> = None;

        for node_info in nodes {
            let node_type_key = node_info.r#type.unwrap_or(0);
            let node_type = match proto_node_type_to_domain(node_type_key) {
                Some(t) => t,
                None => {
                    ...
                    batch.node_results.push(rm::NodeOperationResult {
                        ...
                        status: rm::ReturnCode::Failure.into(),
                        error_message: "Unknown or missing node type".into(),
                    });
                    skipped_nodes += 1;
                    continue;
                }
            ...
            let node = match build_ephemeral_node(&node_info, node_type) {
                Ok(n) => n,
                Err(e) => {
                    let message = e.message;
                    tracing::error!(
            ...
            all_child_ids.push(job_id.clone());
        ...
        let jobs_created = all_child_ids.len() as u32;
        if jobs_created == 0 {
        ...
        let parent_rack_id = parent_rack_id.unwrap_or_default();
        let parent_id = self
            .job_tracker
            .create_parent_job(&parent_rack_id, &all_child_ids, JobType::FirmwareUpdate)
            .unwrap_or_default();
...
mod tests {
    ...
    async fn update_firmware_reports_failure_when_nvue_tls_initialization_fails() -> TestResult {
        ...
        assert!(response.message.contains("--default-switch-domain"));
    ...
    async fn firmware_job_status_bridges_switch_image_jobs() -> TestResult {
        ...
        service
            .job_tracker
            .mark_failed(&job_id, JobError::Other, "switch image update failed");
    ...
    impl Node for ActivationNode {
        ...
        async fn activate_firmware_with(
            ...
            let expected = match self.node_type {
                NodeType::ComputeGb200Nvidia
                | NodeType::ComputeGb300Nvidia
                | NodeType::ComputeGb300Lenovo => FirmwareActivationMode::FullGb200Compute,
                NodeType::SwitchGb200Nvidia | NodeType::SwitchGb300Nvidia => {
                    FirmwareActivationMode::SwitchPowerCycle
                }
                NodeType::PowershelfGb200Liteon
                | NodeType::PowershelfGb200Delta
                | NodeType::PowershelfGb300Liteon
    ...
    async fn spawned_firmware_job_uses_completed_success_status() {
        ...
        let pending = tracker
            .create_pending_job("rack-1", "node-1", JobType::FirmwareUpdate)
            .unwrap();
        let job_id = pending.id().to_string();
        ...
        assert_eq!(
            job.state,
            crate::orchestrator::job_tracker::JobState::Completed
        );
        ...
        tokio::time::timeout(Duration::from_secs(1), async {
            loop {
                ...
                if matches!(
                    job.state,
                    crate::orchestrator::job_tracker::JobState::Completed
                        | crate::orchestrator::job_tracker::JobState::Failed
                ) {
                    return job;
                }
