...
use crate::api::grpc::conversions::{
    flatten_node_info, power_state_to_string, proto_node_type_to_domain,
};
...
fn build_ephemeral_power_node(
    ...
    let node_type = proto_node_type_to_domain(node_info.r#type.unwrap_or(0))
        .ok_or_else(|| "Unknown or missing node type".to_owned())?;
    ...
    if node_type.kind() == NodeKind::Switch {
        ...
        let dangerously_accept_invalid_certs = bmc.dangerously_accept_invalid_certs;
    ...
    let config = NodeConfig {
        id: node_info.node_id.clone(),
        node_type,
        bmc_endpoint: Some(flat.bmc_endpoint().map_err(|e| e.message)?),
