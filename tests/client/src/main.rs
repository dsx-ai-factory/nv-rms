...
type Client = RackManagerClient<Channel>;
...
#[command(
  ...
  rack_manager_client localhost 8801 firmware_object add gb200 /tmp/sot.json $ARTIFACTORY_TOKEN --set-default
  rack_manager_client localhost 8801 firmware_object apply rack-1 /tmp/nodes.csv prod gb200 --component-filter 2:BMC --dangerously-accept-invalid-certs
  ...
  rack_manager_client localhost 8801 switch device_info_batch /tmp/switches.csv --dangerously-accept-invalid-certs"
)]
struct Cli {
...
enum Command {
    ...
    /// Firmware-object / SOT management commands
...
enum FirmwareObjectCommand {
    /// Add a firmware object from a SOT JSON file
    Add {
    ...
    /// Parse/download/apply firmware object targets from a SOT JSON file without persisting it
    #[command(name = "apply")]
    Apply {
        rack_id: String,
        nodes_csv: PathBuf,
    ...
    /// Parse/download/apply a switch system image from a SOT JSON file without persisting it
...
enum SwitchCommand {
    ...
    /// Upgrade switch firmware. Args: <rack_id> [node_id] <component_type> <filename>
    #[command(name = "upgrade_firmware")]
    UpgradeFirmware {
    ...
    /// CSV: node_id,rack_id,ip,port,username,password[,mac_address][,host_name]
...
async fn build_client(cli: &Cli) -> ClientResult<Client> {
    ...
    Ok(RackManagerClient::new(channel))
}
...
async fn run(cli: Cli) -> ClientResult<()> {
    ...
    match cli.command {
        ...
        } => add_nodes(&mut client, &devices_csv, dangerously_accept_invalid_certs).await?,
        ...
        Command::BatchGetPowerState {
            nodes_csv,
            dangerously_accept_invalid_certs,
        } => {
            batch_get_power_state(&mut client, &nodes_csv, dangerously_accept_invalid_certs).await?
        }
...
async fn switch_command(client: &mut Client, command: SwitchCommand) -> ClientResult<()> {
    match command {
        ...
        SwitchCommand::UpgradeFirmware { args } => upgrade_firmware(client, args).await,
        SwitchCommand::ClusterState {
            target_csv,
            dangerously_accept_invalid_certs,
        } => cluster_state(client, &target_csv, dangerously_accept_invalid_certs).await,
        ...
        SwitchCommand::AppStatus {
            targets_csv,
            dangerously_accept_invalid_certs,
        } => app_status(client, &targets_csv, dangerously_accept_invalid_certs).await,
        ...
        SwitchCommand::DeviceInfoBatch {
            targets_csv,
            dangerously_accept_invalid_certs,
        } => {
            batch_get_node_device_info(client, &targets_csv, dangerously_accept_invalid_certs).await
        }
    }
}
...
async fn batch_get_node_device_info(
    ...
    let nodes = parse_switch_targets_csv(targets_csv)?
        .into_iter()
        .map(|target| switch_target_to_node(target, dangerously_accept_invalid_certs))
        .collect();
    let response = client
...
async fn batch_set_power_state(
    ...
    let nodes = parse_node_list_csv(nodes_csv)?
        ...
        .map(|node| {
            node_list_entry_to_proto(
                node,
                NodeListEndpointMode::DirectOperation,
                dangerously_accept_invalid_certs,
            )
        })
        .collect();
...
async fn update_switch_system_password(
    ...
    let common = response
        .response
        .ok_or("update_switch_system_password response missing common response")?;
    print_status(common.status);
    println!("Message: {}", common.message);
    print_node_operation_stats(&common.stats);
    print_common_node_results(&common.node_results);
...
async fn upgrade_firmware(client: &mut Client, args: Vec<String>) -> ClientResult<()> {
    let (rack_id, node_id, component, filename) = parse_upgrade_firmware_args(args)?;
    let response = client
        .upgrade_switch_firmware(rm::UpgradeSwitchFirmwareRequest {
            rack_id,
    ...
    if response.status == rm::ReturnCode::Success as i32 {
        ...
        if !response.current_filename.is_empty() {
            println!("Previous filename: {}", response.current_filename);
        }
        if !response.new_filename.is_empty() {
            println!("New filename: {}", response.new_filename);
        }
    } else {
        println!("Firmware upgrade failed at stage {}", response.failed_stage);
        if !response.error_message.is_empty() {
            println!("Error: {}", response.error_message);
        }
    }
    if !response.result_json.is_empty() {
        println!("Full upgrade details:");
        println!("{}", response.result_json);
    }
    ensure_success(response.status, "switch upgrade_firmware failed")
}
...
async fn cluster_state(
    ...
    let response = client
        .get_scale_up_fabric_state(rm::GetScaleUpFabricStateRequest {
            node: Some(switch_target_to_node(
                target,
                dangerously_accept_invalid_certs,
            )),
        })
...
fn parse_switch_targets_csv(path: &Path) -> ClientResult<Vec<SwitchTarget>> {
    ...
    for (line_number, fields) in csv_records(path)?.into_iter().enumerate() {
        if fields.len() < 6 {
            return Err(format!(
                "invalid CSV format on line {}. Expected node_id,rack_id,ip,port,username,password[,mac_address][,host_name]",
                line_number + 1
            )
            .into());
        }
...
fn is_switch_node_type(node_type: i32) -> bool {
    matches!(
        ...
        Ok(rm::NodeType::SwitchGb200Nvidia | rm::NodeType::SwitchGb300Nvidia)
    )
}
...
fn node_list_entry_to_proto(
    node: NodeListEntry,
    mode: NodeListEndpointMode,
    dangerously_accept_invalid_certs: bool,
) -> rm::NodeInfo {
    ...
    if matches!(
        rm::NodeType::try_from(node.node_type),
        Ok(rm::NodeType::SwitchGb200Nvidia | rm::NodeType::SwitchGb300Nvidia)
    ) {
...
fn parse_upgrade_firmware_args(
    args: Vec<String>,
) -> ClientResult<(String, Option<String>, i32, String)> {
    match args.as_slice() {
        [rack_id, component, filename] => {
            let component = parse_component_type(component)?;
            Ok((rack_id.clone(), None, component, filename.clone()))
        }
        [rack_id, node_id, component, filename] => {
            let component = parse_component_type(component)?;
            Ok((
                rack_id.clone(),
                Some(node_id.clone()),
        ...
        _ => Err(
            "usage: switch upgrade_firmware <rack_id> [node_id] <component_type> <filename>".into(),
        ),
    }
}
...
fn validate_node_type(node_type: i32) -> ClientResult<()> {
    if matches!(
        rm::NodeType::try_from(node_type),
        Ok(rm::NodeType::ComputeGb200Nvidia
        ...
        Err("node_type must be a supported NodeType enum value".into())
    }
}
...
fn parse_node_type_field(value: &str, context: &str) -> ClientResult<i32> {
    ...
    if let Some(node_type) = rm::NodeType::from_str_name(&value.to_ascii_uppercase()) {
        let node_type = node_type as i32;
        validate_node_type(node_type)?;
        return Ok(node_type);
    }

    let normalized = value.to_ascii_lowercase().replace(['-', ' '], "_");
    for node_type in 1i32..=9 {
...
fn parse_component_type(value: &str) -> ClientResult<i32> {
    let component_type = value
        .parse::<i32>()
        .map_err(|_| "component_type must be a number")?;
    validate_component_type(component_type)?;
    Ok(component_type)
}
...
mod tests {
    ...
    fn direct_operation_switch_bmc_role_uses_bmc_endpoint_only() {
        ...
        let node = node_list_entry_to_proto(entry, NodeListEndpointMode::DirectOperation, false);
    ...
    fn inventory_registration_switch_bmc_role_uses_bmc_endpoint_only() {
        ...
        let node =
            node_list_entry_to_proto(entry, NodeListEndpointMode::InventoryRegistration, false);
    ...
    #[test]
    fn direct_operation_preserves_invalid_cert_override() {
        let node = node_list_entry_to_proto(
            switch_node_entry(),
            NodeListEndpointMode::DirectOperation,
            true,
    ...
    fn switch_target_to_node_populates_host_name() {
        let node = switch_target_to_node(
            SwitchTarget {
                node_id: "sw-01".into(),
    ...
    fn parse_node_list_accepts_switch_host_name_with_bmc_columns() -> ClientResult<()> {
        ...
        std::fs::write(
            csv.path(),
            "sw-01,rack-01,10.0.0.1,443,nvue-user,nvue-pass,aa:bb:cc:dd:ee:ff,6,,sw-01.switch.example.com,192.168.1.100,8443,bmc-user,bmc-pass\n",
        )?;
