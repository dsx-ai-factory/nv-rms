...
async fn start_server_with_manager(
    rm: Arc<RackManager>,
) -> (E2eServer, RackManagerClient<tonic::transport::Channel>) {
    let firmware_dir = TempDir::new().expect("create firmware TempDir");
    let jt = Arc::new(JobTracker::new());
    let port = portpicker::pick_unused_port().expect("no free port");
    let metrics_registry = metrics::init(&metrics::RmsConfig {
        grpc_port: port,
    ...
    let mut grpc_server = GrpcServer::new(port, rm, jt, Backends::memory(), metrics_registry)
...
async fn start_server() -> (E2eServer, RackManagerClient<tonic::transport::Channel>) {
...
#[tokio::test]
async fn create_nodes_unknown_type_fails() {
    let (mut server, mut client) = start_server().await;
    ...
    let response = operation_response(&resp.response);
    assert_eq!(response.status, ReturnCode::Failure as i32);
    assert!(response.message.contains("unknown type"));
...
async fn powershelf_set_power_state() {
    ...
    // Powershelf only has discrete On / ForceOff actions on the wire, but the
    // node folds graceful Off into ForceOff and ForceOn into On since there is
    // no separate hardware surface for either.
    for op in [
...
async fn batch_update_firmware_missing_type_reports_per_node_failure() {
    ...
    // No jobs should be created (no matching targets), so the RPC must
    // report FAILURE with 0 successful updates and a non-empty per-node
...
#[tokio::test]
async fn upgrade_switch_firmware_rejects_existing_file_outside_firmware_dir() {
    let (mut server, mut client) = start_server().await;
    add_nodes(
        &mut client,
        vec![switch_device(
    ...
    let resp = client
        .upgrade_switch_firmware(UpgradeSwitchFirmwareRequest {
            rack_id: "rack-01".into(),
            node_id: "sw-01".into(),
            component_type: SwitchFirmwareComponentType::Bmc as i32,
            filename: image.to_str().unwrap().into(),
        })
        .await
        .unwrap()
        .into_inner();
...
//
//  All ScaleUp RPCs now take a caller-supplied NodeInfo (no inventory
//  lookup). These tests cover validation + empty-nodes handling; full
