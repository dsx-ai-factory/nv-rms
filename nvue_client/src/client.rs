...
impl Client {
    ...
    pub async fn get(&self, path: &str, timeout: Duration) -> Result<NvueResponse, ClientError> {
        self.send(Method::GET, path, None, None, timeout).await
    }
    ...
    pub async fn prepare_client_tls(
        ...
        let prepared = self
            .prepare_client_tls_candidate(tls, server_name, false)
            .await?;
    ...
    pub async fn prepare_client_tls_if_changed(
        ...
        self.prepare_client_tls_candidate(tls, server_name, true)
            .await
    }
    ...
    pub async fn activate_prepared_client_tls(
        ...
        let response = active_transport
            ...
            .send(Method::GET, path, None, None, timeout)
            .await?
            .into_json("GET", path)?;
...
fn build_plain_transport(
    ...
    let mut builder = disable_proxies(reqwest::Client::builder())
    ...
    if normalize_host(&config.endpoint.host) != normalize_host(&config.endpoint.connect_host) {
        ...
        builder = builder.resolve(normalize_host(&config.endpoint.host), socket_addr);
    }

    let client = builder.build().map_err(|source| ClientError::Request {
