...
fn target_input_option_supported(key: &str) -> bool {
    matches!(
        key,
        "ip" | "port" | "user" | "password" | "servertype" | "verify_tls" | "bmc_ca_cert"
    )
}
...
impl<'a> FwUpdCmdBase<'a> {
    ...
    pub fn validate_os_target_json(
        ...
        for input_opt in args {
            ...
            if parts.len() == 2 {
                let key = parts[0].to_lowercase();
                if !["ip", "port", "user", "password", "servertype"].contains(&key.as_str()) {
                    Util::bail_nvfwupd(
                        1,
                        &format!(
...
fn create_rf_target(
    ...
    match class_name {
        ...
        "MGXNVLRFTarget" => Box::new(MGXNVLRFTarget::new(bmc_access, config_dict)),
...
mod tests {
    ...
    #[test]
    fn target_input_option_support_includes_tls_options() {
        for key in [
            "ip",
            "port",
            "user",
        ...
        for key in ["ca_cert", "tls_ca", "verify_ssl", "unknown"] {
            assert!(
                !target_input_option_supported(key),
