...
const GB200_FUNGIBLE_FRAGMENTS: &[&str] = &[
    "fw_io_board_sma",
...
const BACKGROUND_COPY_QUERY_TIMEOUT_SECS: u64 = 5;
...
impl RFTarget for GB200RFTarget {
    ...
    async fn get_identifier_from_chassis(&self, ap_inv_uri: &str) -> Option<String> {
        ...
        if ap_lower.contains("fw_io_board_sma")
            || ap_lower.contains("fw_processormodule_sma")
            || ap_lower.contains("fw_gpu_sma")
        {
...
mod tests {
    ...
    fn make_pldm_version_dict() -> Value {
        json!({
            "GB200-P4972_0009-HGX": {
                ...
                "NVMe:Dell_Ent_NVMe_v2_AGN:,0x22220001": ["2.1.0", "dell_ent_nvme_v2_agn"],
    ...
    async fn test_get_component_version_nvme() {
        ...
        let target = GB200RFTarget::new(bmc, None);
        let dict = make_pldm_version_dict();
        // "NVMe:Dell_..." normalizes to "nvme", AP "HGX_FW_NVMe_E1S_0" → "nvmee1s0"
    ...
    fn test_get_version_sku_nvme_by_model() {
        ...
        let target = GB200RFTarget::new(bmc, None);
        let dict = make_pldm_version_dict();
        let result = target.get_version_sku("Dell_Ent_NVMe_v2_AGN", &dict, "hgx_fw_nvme_e1s_0");
        assert_eq!(result, Some("2.1.0".to_string()));
    }
