...
const SWITCH_SERVER_TYPES: &[&str] = &["gb200switch", "gb300switch", "vrswitch", "vrnvl72switch"];

/// Platform-to-model mapping used during Chassis discovery.
const PLATFORM_DICT: &[(&str, &str)] = &[
    ("P4352_Management_Board", "Falcon"),
    ("P4351_Management_Board", "Starship"),
    ("E4231_Management_Board", "ut2.1 dp chassis"),
    ("P4265_Management_Board", "Ranger_Keystone"),
    ("E4870_Management_Board", "E4870"),
    ("E4866_Management_Board", "E4866"),
    ("E4869_Management_Board", "E4869"),
...
static HTTP_RUNTIME: Lazy<tokio::runtime::Runtime> = Lazy::new(|| {
...
fn tls_certificate_verification_message_for_detail(url: &str, detail: &str) -> Option<String> {
    ...
    Some(format!(
        "Connection Error: TLS certificate verification failed while connecting to {}. \
         Certificate validation uses built-in public roots plus the host trust store by default; \
         pass bmc_ca_cert=/path/to/ca.pem to trust a per-command CA, or pass verify_tls=false \
         only when validation must be disabled. \
         Details: {}",
...
impl BmcAccess {
    ...
    /// Build a [`reqwest::Client`] with certificate validation enabled by default.
    ///
    /// The default Rustls verifier uses built-in public roots plus the host
    /// trust store. Operators can opt out for lab/self-signed endpoints by
    /// passing `verify_tls=false` in the target arguments. Passing
    /// `bmc_ca_cert=/path/to/ca.pem` keeps validation enabled and adds that PEM
    ...
    /// Async variant of [`get_bmc_access_sync`].
    ...
    async fn get_firmware_inventory_redfish_async(
        ...
        let is_powershelf_by_attr = self.servertype == "powershelf";
        let model_hint = model.unwrap_or(self.model.as_str());
        let model_lower = model_hint.to_ascii_lowercase();
        let is_megmeet_powershelf = model_lower.contains("mpsc") || model_lower.contains("megmeet");
        let is_powershelf_by_model = matches!(
            model_lower.as_str(),
            "pf-1333-13rd" | "pf-1333-7r" | "pf-1114-1r"
        );

        if (is_powershelf_by_attr || is_powershelf_by_model) && !is_megmeet_powershelf {
...
mod tests {
    ...
    #[test]
    fn bmc_tls_verification_defaults_to_secure_mode() {
    ...
    fn test_switch_server_types() {
        for st in SWITCH_SERVER_TYPES {
            assert!(["gb200switch", "gb300switch", "vrswitch", "vrnvl72switch"].contains(st));
        }
    }
}
