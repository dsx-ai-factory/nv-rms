...
//! HGXB100RFTarget, MGXNVLRFTarget, and HGXRUBINRFTarget implementations.
//!
//! - `HGXB100RFTarget` -- HGX B100/B300 platforms (multipart upload).
//! - `MGXNVLRFTarget` -- MGX NVL minimal wrapper.
...
// ---------------------------------------------------------------------------
// Shared helpers for HGXB100/MGXNVL/HGXRUBIN
// ---------------------------------------------------------------------------
...
/// Minimal RFTarget wrapper for MGX NVL platforms.
///
/// Behaves identically to HGXB100RFTarget with a distinct class name
/// for platform identification.
pub struct MGXNVLRFTarget {
    /// BMC connection handle.
    pub bmc_access: BmcAccess,
    /// Component names treated as fungible.
    pub fungible_components: Vec<String>,
...
impl MGXNVLRFTarget {
    /// Create a new `MGXNVLRFTarget` with sensible defaults.
    pub fn new(bmc_access: BmcAccess, config_dict: Option<Value>) -> Self {
        Self {
            bmc_access,
...
#[async_trait::async_trait]
impl RFTarget for MGXNVLRFTarget {
    fn target_access(&self) -> &BmcAccess {
        &self.bmc_access
    }
    ...
    // MGXNVL inherits get_update_uri from trait default:
    // "/redfish/v1/UpdateService" (HttpPushUri), matching Python.

    /// MGX NVL: GH-style PATCH + push-URI upload with `cmd_args.special`.
    async fn update_component(
        &mut self,
        cmd_args: &CmdArgs,
        update_uri: &str,
    ...
    fn is_fungible_component(&self, component_name: &str) -> bool {
        // MGXNVL Python: excludes "inforom", "erot", AND "driver"
        let lower = component_name.to_lowercase();
        self.fungible_components
            .iter()
            .any(|f| lower.contains(f.as_str()))
