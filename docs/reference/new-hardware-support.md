...
1. Is it a new node role, or one of the existing roles: compute, powershelf, or switch?
1. Is it a new vendor-specific node type inside an existing product family?
1. Does it use the BMC endpoint, the host endpoint, or both?
1. Does NVFWUPD already have an appropriate `ServerType`, target class, and firmware package behavior?
1. Does firmware-object apply need target remapping, component filtering, or multiple firmware packages for one logical target?
1. Can it safely delegate to an existing implementation, or does it need a new concrete node implementation?

## API and Node Identity

Unless stated otherwise, paths in this guide are relative to
`crates/rackmanagementservice`.
...
The following NVFWUPD paths are relative to `crates/rust_nvfwupd`.

- Confirm `src/workflow.rs` has the correct `ServerType`.
- Add target aliases in `src/rf_target.rs` if package metadata or
  CLI input needs to resolve to an existing target implementation.
- Add or update target behavior in `src/workflow_api.rs` when the
...
1. Add one node with the new `NodeType` and the expected endpoint shape.
1. List inventory and confirm the returned node type.
1. Run a low-risk power read.
1. Run firmware inventory.
1. Verify firmware package versions for a known-good package.
1. Exercise the narrowest safe firmware update or activation flow in a controlled lab.

## Documentation and Release Checklist

Update documentation with the same branch as the hardware support code:

- Add the new platform to the [Hardware Compatibility List](hcl.md) when support is ready.
